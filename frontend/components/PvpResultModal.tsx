...
type Props = {
  result: PvpBattle | null;
  onClose: () => void;
};

export default function PvpResultModal({ result, onClose }: Props) {
  if (!result) return null;

  const won = result.winner_id === result.attacker_id;
  const trophyDelta = result.attacker_trophies_delta;
      ...
      <View style={styles.overlay}>
        <View style={styles.card}>
          <View style={[styles.header, won ? styles.headerWin : styles.headerLose]}>
            <Text style={styles.title}>{won ? "⚔️ Zafer!" : "💀 Yenilgi"}</Text>
            <View style={styles.rewardRow}>
              <Text style={won ? styles.reward : styles.noReward}>
                {trophyDelta >= 0 ? "+" : ""}{trophyDelta} 🏆
              </Text>
              <Text style={styles.xp}>vs {result.defender_name}</Text>
            </View>
          </View>

          <View style={{ paddingHorizontal: 16, paddingTop: 12, gap: 8, flexDirection: "row", alignItems: "center", justifyContent: "center" }}>
            {(["strawberry", "pinecone", "blueberry"] as const).map((r) => {
              const amt = (result as any)[`transferred_${r}`] ?? 0;
              ...
              return (
                <View key={r} style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  ...
                  <Text style={[styles.reward, { color: won ? "#4a7c3f" : "#c0392b" }]}>
                    {won ? "+" : "-"}{amt}
                  </Text>
                </View>
              );
          ...
          {(result.combat_log?.length ?? 0) > 0 && (
            <>
              ...
              <ScrollView style={styles.logScroll} showsVerticalScrollIndicator={false} nestedScrollEnabled>
                {result.combat_log.map((entry: any, i: number) => {
                  ...
                  const newRound = i === 0 || result.combat_log[i - 1]?.round !== entry.round;
                  const atkName = isChamp ? result.attacker_champion_name : result.defender_champion_name;
                  const defName = isChamp ? result.defender_champion_name : result.attacker_champion_name;
