...
type Props = {
  confirm: CapUpgradeConfirm | null;
  resources: Resources;
  onClose: () => void;
  onConfirm: (resource: ResourceKey) => Promise<void>;
};

export default function CapUpgradeModal({ confirm, resources, onClose, onConfirm }: Props) {
  const { t } = useLanguage();

  if (!confirm) return null;

  const { resource, currentCap, cost, costRes1, costRes2 } = confirm;
  const meta = RESOURCE_META[resource];
  const meta1 = RESOURCE_META[costRes1];
  const meta2 = RESOURCE_META[costRes2];
  const canAfford = resources[costRes1] >= cost && resources[costRes2] >= cost;
      ...
      <View style={styles.overlay}>
        <View style={styles.card}>
          ...
          <View style={styles.resourceRow}>
            <Image source={meta.image} style={styles.resIcon} resizeMode="contain" />
          ...
          <View style={styles.costRow}>
            <View style={styles.costItem}>
              ...
              <Text style={[styles.costText, resources[costRes1] < cost && styles.costLow]}>
                ×{cost}
              </Text>
            </View>
            ...
            <View style={styles.costItem}>
              ...
              <Text style={[styles.costText, resources[costRes2] < cost && styles.costLow]}>
                ×{cost}
              </Text>
            </View>
          </View>
