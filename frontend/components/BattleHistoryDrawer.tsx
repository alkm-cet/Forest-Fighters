...
import type { PvpBattle, ClaimResult, GearSnapshot, PlayerGear } from "../types";
...
function normalize(props: PvpProps | PveProps): Normalized {
  if (props.mode === "pvp") {
    ...
    const leftName  = isAttacker ? battle.attacker_champion_name : battle.defender_champion_name;
    const leftClass = isAttacker ? battle.attacker_champion_class : battle.defender_champion_class;
    const rightName  = isAttacker ? battle.defender_champion_name : battle.attacker_champion_name;
    const rightClass = isAttacker ? battle.defender_champion_class : battle.attacker_champion_class;
    ...
    return {
      won,
      leftName, leftClass, leftGear,
      rightName, rightClass, rightGear,
      log,
      trophyDelta,
  ...
  if (!gear) {
    return (
      <View style={{ marginTop: 4, opacity: 0.45, alignItems: align === "right" ? "flex-end" : "flex-start" }}>
        <Text style={{ fontSize: 10, color: "#888" }}>— ekipman bilgisi yok —</Text>
      </View>
    );
  }
...
function ChampionPortrait({ name, cls, gear, align }: { name: string; cls: string | null; gear: GearSnapshot | null; align?: "left" | "right" }) {
  const classMeta = cls ? CLASS_META[cls] : null;
  return (
    <View style={[portraitStyles.container, { alignItems: align === "right" ? "flex-end" : "flex-start" }]}>
      <View style={[portraitStyles.imageWrap, { borderColor: classMeta?.color ?? "#bbb" }]}>
        {classMeta ? (
...
function PvpOutcome({ n, won }: { n: Normalized; won: boolean }) {
  ...
  const resources = (["strawberry", "pinecone", "blueberry"] as const).filter(
    (r) => ((n as any)[`transferred${r.charAt(0).toUpperCase() + r.slice(1)}`] ?? 0) > 0
  );
  return (
      ...
      {(["strawberry", "pinecone", "blueberry"] as const).map((r) => {
        ...
        return (
          <View key={r} style={outcomeStyles.resourceItem}>
            ...
            <Text style={[outcomeStyles.resourceText, { color: won ? "#2e7d32" : "#c62828" }]}>
              {won ? "+" : "-"}{amt}
            </Text>
          </View>
        );
    ...
    <View style={{ gap: 8 }}>
        ...
        {(n.rewardAmount ?? 0) > 0 && n.rewardResource && (
          <View style={outcomeStyles.resourceItem}>
            {RESOURCE_META[n.rewardResource]?.image && (
              <Image source={RESOURCE_META[n.rewardResource].image} style={{ width: 20, height: 20 }} resizeMode="contain" />
            )}
        ...
        {(n.rewardAmount2 ?? 0) > 0 && n.rewardResource2 && (
          <View style={outcomeStyles.resourceItem}>
            {RESOURCE_META[n.rewardResource2!]?.image && (
              <Image source={RESOURCE_META[n.rewardResource2!].image} style={{ width: 20, height: 20 }} resizeMode="contain" />
            )}
      ...
      {(n.levelsGained ?? 0) > 0 && (
        <View style={outcomeStyles.levelUpBadge}>
          <Text style={outcomeStyles.levelUpText}>⬆️ SEVİYE ATLADI! LV {n.newLevel}</Text>
        </View>
      )}
      {/* Gear drops */}
...
function StartHpRow({ log, leftName, rightName }: { log: CombatEntry[]; leftName: string; rightName: string }) {
  if (log.length === 0) return null;
  const first = log[0];
  // Derive starting HP: whoever was hit in the first entry had HP reduced by damage
  const leftStartHp  = first.actor === "right" ? first.leftHpAfter  + first.damage : first.leftHpAfter;
  const rightStartHp = first.actor === "left"  ? first.rightHpAfter + first.damage : first.rightHpAfter;
  return (
...
function MiniPortrait({ cls }: { cls: string | null }) {
      ...
      <View style={[logStyles.line, leftAttacks ? logStyles.lineLeftAtk : logStyles.lineRightAtk]}>
        ...
        <View style={[logStyles.side, leftTookDamage && logStyles.sideDamaged]}>
          <View style={logStyles.nameRow}>
            ...
            <Text style={[logStyles.sideName, { color: leftAttacks ? "#2d6e24" : "#333" }]} numberOfLines={1}>
              {leftName}
            </Text>
          </View>
          <Text style={[logStyles.hpText, leftTookDamage && logStyles.hpDamaged]}>
            {leftHp} HP
          </Text>
          {leftAttacks ? (
            <View style={logStyles.statBadgeRow}>
              ...
              {entry.atkBoosted && (
                <View style={logStyles.critBadge}><Text style={logStyles.critText}>⚡KRİT</Text></View>
              )}
            </View>
          ) : (
            <View style={logStyles.statBadgeRow}>
              {entry.defBoosted && (
                <View style={logStyles.blockBadge}><Text style={logStyles.blockText}>🛡KALKAN</Text></View>
              )}
        ...
        <View style={[logStyles.side, logStyles.sideRight, rightTookDamage && logStyles.sideDamaged]}>
          <View style={[logStyles.nameRow, { justifyContent: "flex-end" }]}>
            <Text style={[logStyles.sideName, { color: !leftAttacks ? "#a02020" : "#333" }]} numberOfLines={1}>
              {rightName}
            </Text>
            <MiniPortrait cls={rightClass} />
          </View>
          <Text style={[logStyles.hpText, logStyles.hpRight, rightTookDamage && logStyles.hpDamaged]}>
            {rightHp} HP
          </Text>
          {!leftAttacks ? (
...
export default function BattleHistoryDrawer(props: Props) {
  ...
  const subtitleText = props.mode === "pvp"
    ? `vs ${props.battle.attacker_id === props.myPlayerId ? props.battle.defender_name : props.battle.attacker_name}`
    : props.result.winner === "champion" ? "Dungeon temizlendi!" : "Düşman çok güçlüydü";
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <View style={styles.overlay}>
        <View style={styles.card}>
          {/* ── HEADER ── */}
          <View style={[styles.header, n.won ? styles.headerWin : styles.headerLose]}>
            <Text style={styles.title}>{n.won ? "⚔️  ZAFER!" : "💀  BOZGUN"}</Text>
          ...
          <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.vsSection}>
              ...
              <View style={{ flex: 1, alignItems: "flex-end" }}>
                <ChampionPortrait name={n.rightName} cls={n.rightClass} gear={n.rightGear} align="right" />
              </View>
            </View>
            ...
            {n.log.length > 0 && (
              <View style={styles.logSection}>
                ...
                <StartHpRow log={n.log} leftName={n.leftName} rightName={n.rightName} />
                {n.log.map((entry, i) => (
                  <CombatLogEntry
                    key={i}
                    entry={entry}
